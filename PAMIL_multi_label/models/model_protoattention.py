...
class PAMIL(nn.Module):        
    ...
    def forward(self, h, inst_pred=False, label=None):
        ...
        if self.proto_pred:
            ...
            proto_score = proto_score / torch.max(proto_score)  # add for version 28
...
# def push(loader, model, epoch, save_proto=False, result_dir=None, cur=None):  # v24
#     # get the proto_A
#     device=torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
def save_push_result(epoch, patients, coords, save_path=None, cur=None):
